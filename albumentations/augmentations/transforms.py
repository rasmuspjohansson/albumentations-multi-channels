    ...
    def apply(self, image, vertices_list=(), **params):
        ...
        if image.shape[-1]>3:
            if image.shape[-1]>7:
                ...
                image[:,:,4:8] = self.apply_shadow_to_four_channel_image(image[:,:,4:8],vertices_list=vertices_list_2)
            else:
...
class ChannelValueAugmentation(ImageOnlyTransform):
    ...
    def apply(self, img, **params):
        values_to_ad = np.array([random.uniform(min,max) for (min,max) in self.addition_ranges])
        values_to_multiply = np.array([random.uniform(min,max) for (min,max) in self.multiplicative_ranges])
